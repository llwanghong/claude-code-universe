...
这个架构设计的核心概念是"**节**"（Section）——system prompt 不是一整坨文本，而是由多个独立的"节"拼接而成。你可以把每个节想象成乐高积木：一块是"告诉模型它是谁"，一块是"教模型怎么用工具"，一块是"设置语气风格"。每个节是一段独立的 prompt 文本，有自己的 `cacheScope`（缓存范围）属性，决定它放在边界的哪一侧。

为什么要拆成节？三个原因：（1）**可组合**——不同功能团队维护自己的节，不会互相踩脚；（2）**缓存控制**——静态节（对所有人相同）放边界之前享受全局缓存，动态节（每用户不同）放边界之后用会话缓存；（3）**可追踪**——每个节有显式的名称和理由，出了问题知道是哪个节引起的。

节的命名约定故意很响亮：添加新节时，必须在 `systemPromptSection`（安全，可缓存，放边界之前）和 `DANGEROUS_uncachedSystemPromptSection`（破坏全局缓存，需要理由字符串，放边界之后）这两个工厂函数之间选择。`_reason` 参数在运行时不会报错，但作为强制性文档存在——每个可能破坏缓存的节都在源码中带着它存在的理由。
