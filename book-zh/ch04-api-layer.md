...
> 💡 **译注**：这个设计的精妙之处在于"猜测成本"。绝大多数模型的单次响应在 8000 token 以内，所以默认预留 8K 就够了。但如果模型这次需要长篇输出（比如生成一个完整的代码文件），8K 不够用，它会在生成中途被截断。此时系统记下"这次需要更多空间"，自动把限额提到 64K 重新请求。关键点：这 64K 不是免费的——更大的输出窗口意味着 API 要预留更多计算资源，成本更高。所以系统宁可先猜 8K（省钱），猜错了再升级（贵但必要）。99% 的情况猜对了。
