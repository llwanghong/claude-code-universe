...
> 💡 **译注**：这个优化的意义可以类比：如果你在 60fps 的游戏中，每帧要画 24,000 个小方块。方案 A 是每帧 new 24,000 个对象（JavaScript 对象），画完扔掉等 GC 回收；方案 B 是预分配一块固定大小的内存（typed array），每帧只修改里面的数值。方案 A 不仅占 3MB 内存，而且 GC 每 16ms 就要跑一次，导致渲染卡顿。方案 B 只占 64KB，且无 GC —— 只改数值，不改结构。终端里"流式输出 token"本质上就是高频更新这些单元格。
