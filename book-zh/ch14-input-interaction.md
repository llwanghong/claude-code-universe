...
**CSI u（Kitty 键盘协议）** 是现代标准。格式：`ESC [ codepoint [; modifier] u`。示例：`ESC[13;2u` 是 Shift+Enter，`ESC[27u` 是无修饰键的 Escape。码点无歧义地标识键——Escape-the-key 和 Escape-as-sequence-prefix 之间没有歧义。修饰键字将 shift、alt、ctrl 和 super（Cmd）编码为单独位。Claude Code 在启动时通过 `ENABLE_KITTY_KEYBOARD` escape 序列在支持的终端上启用此协议，并在退出时通过 `DISABLE_KITTY_KEYBOARD` 禁用它。协议通过查询/响应握手检测：应用发送 `CSI ? u`，终端响应 `CSI ? flags u`，其中 `flags` 指示支持的协议级别。

**xterm modifyOtherKeys** 是 Kitty 协议未协商时 Ghostty over SSH 等终端的后备。格式：`ESC [ 27 ; modifier ; keycode ~`。注意参数顺序与 CSI u 相反——修饰键在键码之前。这是解析器 bug 的常见来源。协议通过 `CSI > 4 ; 2 m` 启用，由 Ghostty、tmux 和 xterm 发出，当终端的 TERM 标识未被检测到时（常见于 `TERM_PROGRAM` 不被转发的 SSH 连接）。

**遗留终端序列** 覆盖其余一切：通过 `ESC O` 和 `ESC [` 序列的功能键、箭头键、数字键盘、Home/End/Insert/Delete，以及 40 年终端演化积累的完整 VT100/VT220/xterm 变体集合。解析器使用两个正则来匹配这些：`FN_KEY_RE` 用于 `ESC O/N/[/[[` 前缀模式（匹配功能键、箭头键及其修饰变体），`META_KEY_CODE_RE` 用于 meta-key 码（`ESC` 后跟单个字母数字，传统的 Alt+key 编码）。
...
终端识别添加另一层复杂性。启动时，Claude Code 发送 `XTVERSION` 查询（`CSI > 0 q`）来发现终端名称和版本。响应（`DCS > | name ST`）在 SSH 连接中存活——不像 `TERM_PROGRAM`，它是不会通过 SSH 传播的环境变量。知道终端标识允许解析器处理终端特定的怪癖。例如，xterm.js（VS Code 集成终端使用）有与原生 xterm 不同的 escape 序列行为，标识字符串（`xterm.js(X.Y.Z)`）允许解析器解释这些差异。

**SGR 鼠标事件** 使用格式 `ESC [ < button ; col ; row M/m`，其中 `M` 是按下，`m` 是释放。按钮码编码动作：0/1/2 为左/中/右键，64/65 为滚轮上/下（0x40 与滚轮位 OR），32+ 为拖拽（0x20 与运动位 OR）。滚轮事件转换为 `ParsedKey` 对象使它们流经 keybinding 系统；点击和拖拽事件成为路由到选择处理器的 `ParsedMouse` 对象。
...
`kind` 鉴别器确保下游代码显式处理每种输入类型。键不会意外被当作鼠标事件处理；终端响应不会意外被解释为键击。`ParsedKey` 类型还携带原始 `sequence` 字符串用于调试——当用户报告"按下 Ctrl+Shift+A 没有反应"时，调试日志可以确切显示终端发送了什么字节序列，使诊断问题是终端的编码、解析器的识别还是 keybinding 的配置成为可能。

`isPasted` 标志对安全至关重要。当括号粘贴启用时，终端将粘贴内容包装在标记序列中。解析器在结果键事件上设置 `isPasted: true`，keybinding resolver 对粘贴的键跳过 keybinding 匹配。没有它，粘贴包含 `\x03`（raw 字节中为 Ctrl+C）或 escape 序列的文本将触发应用命令。有了它，粘贴的内容被视为字面文本输入，无论其字节内容为何。

解析器还识别终端响应——终端自身响应查询发送的序列。这些包括设备属性（DA1、DA2）、光标位置报告、Kitty 键盘标志响应、XTVERSION（终端标识）和 DECRPM（模式状态）。这些路由到 `TerminalQuerier` 而非输入处理器：
...
解析器之下是结构化的终端 I/O 子系统，在 `ink/termio/` 中：
...
多路复用器包装值得注意。当 Claude Code 在 tmux 内运行时，某些 escape 序列（如 Kitty 键盘协议协商）必须透传到外部终端。tmux 使用 DCS 透传（`ESC P ... ST`）转发它不理解的序列。`osc.ts` 中的 `wrapForMultiplexer` 函数检测多路复用器环境并适当地包装序列。没有它，Kitty 键盘模式将在 tmux 内静默失败，用户永远不会知道为什么他们的 Ctrl+Shift 绑定停止工作。
...
包装 `ParsedKey` 的 `InputEvent` 存在是为了与遗留 `EventEmitter` 路径向后兼容，旧组件可能仍在使用。新组件使用带捕获/冒泡阶段的 DOM 风格键盘事件分发。两条路径从相同的解析键触发，所以它们始终一致——到达 stdin 的键产生恰好一个 `ParsedKey`，它同时生成 `InputEvent`（用于遗留监听器）和 `KeyboardEvent`（用于 DOM 风格分发）。此双路径设计允许从 EventEmitter 模式增量迁移到 DOM 事件模式而不破坏现有组件。
...
用户可以通过 `~/.claude/keybindings.json` 覆盖任何绑定。解析器接受修饰键别名（`ctrl`/`control`、`alt`/`opt`/`option`、`cmd`/`command`/`super`/`win`）、键别名（`esc` → `escape`、`return` → `enter`）、和弦记法（空格分隔步骤如 `ctrl+k ctrl+s`）和 null 动作以取消绑定默认键。null 动作与不定义绑定不同——它显式阻止默认绑定触发，这对想要为终端使用回收键的用户很重要。
...
"last wins"解析策略意味着如果默认绑定和用户绑定都在 `Chat` 上下文中定义了 `ctrl+k`，用户的绑定优先。这在匹配时通过按定义顺序迭代绑定并保持最后匹配来评估，而非在加载时构建覆盖 map。优势：上下文特定的覆盖自然组合。用户可以在 `Chat` 中覆盖 `enter` 而不影响 `Confirmation` 中的 `enter`。
...
实现处理了大多数 chord 系统错过的一个边缘情况：当用户按下 `ctrl+x` 但然后输入一个不是任何 chord 部分的字符会发生什么？没有仔细处理，该字符将被吞掉——chord 拦截器消费了输入，chord 被取消，字符消失了。Claude Code 的 `ChordInterceptor` 在此情况下返回 `chord_cancelled`，导致 pending 输入被丢弃但允许不匹配的字符回退到正常输入处理。字符不会丢失；只有 chord 前缀被丢弃。这匹配用户对 Emacs 风格 chord 前缀的期望行为。
...
从 `idle`，按 `d` 进入 `operator` 状态。从 `operator`，按 `w` 执行带 `w` 动作的 `delete`。再次按 `d`（`dd`）触发行删除。按 `2` 进入 `operatorCount`，所以 `d2w` 变成"删除接下来的 2 个词"。按 `i` 进入 `operatorTextObj`，所以 `di"` 变成"删除引号内"。每个中间状态精确携带下次过渡所需的上下文——不多不少。
...
`fromIdle` 处理器是入口点，覆盖完整 vim 词汇：

- **计数前缀**：`1-9` 进入 `count` 状态，累积数字。`0` 是特殊的——它是"行首"动作，不是计数数字，除非数字已被累积
- **操作符**：`d`、`c`、`y` 进入 `operator` 状态，等待动作或文本对象定义范围
- **查找**：`f`、`F`、`t`、`T` 进入 `find` 状态，等待要搜索的字符
- **G 前缀**：`g` 进入 `g` 状态用于复合命令（`gg`、`gj`、`gk`）
- **替换**：`r` 进入 `replace` 状态，等待替换字符
- **缩进**：`>`、`<` 进入 `indent` 状态（用于 `>>` 和 `<<`）
- **简单动作**：`h/j/k/l/w/b/e/W/B/E/0/^/$` 立即执行，移动光标
- **立即命令**：`x`（删除字符）、`~`（切换大小写）、`J`（合并行）、`p/P`（粘贴）、`D/C/Y`（操作符快捷键）、`G`（到末尾）、`.`（dot-repeat）、`;/,`（查找重复）、`u`（撤销）、`i/I/a/A/o/O`（进入插入模式）

### 动作、操作符和文本对象

**动作**是将键映射到光标位置的纯函数。`resolveMotion(key, cursor, count)` 应用动作 `count` 次，如果光标停止移动则短路（你不能移动到第 0 列以左）。此短路对行尾的 `3w` 很重要——它停在最后一个词而非 wrapping 或报错。

动作按它们如何与操作符交互分类：

- **Exclusive**（默认）——目的地处的字符不包含在范围内。`dw` 删除直到但不包括下一个词的第一个字符
- **Inclusive**（`e`、`E`、`$`）——目的地处的字符包含。`de` 删除通过当前词的最后一个字符
- **Linewise**（`j`、`k`、`G`、`gg`、`gj`、`gk`）——当与操作符使用时，范围扩展到覆盖完整行。`dj` 删除当前行和下面一行，不仅仅是两个光标位置之间的字符

**操作符**应用于范围。`delete` 移除文本并保存到寄存器。`change` 移除文本并进入插入模式。`yank` 复制到寄存器。`cw`/`cW` 特殊情况遵循 vim 约定：change-word 到当前词尾，而非下一个词首（不同于 `dw`）。

一个有趣的边缘情况：`[Image #N]` chip snapping。当词动作落在图片引用 chip 内部（在终端中渲染为单一视觉单元），范围扩展到覆盖整个 chip。这防止了用户感知为原子元素的部分删除——你不能删除 `[Image #3]` 的一半，因为动作系统将整个 chip 视为单个词。

额外命令覆盖完整的预期 vim 词汇：`x`（删除字符）、`r`（替换字符）、`~`（切换大小写）、`J`（合并行）、`p`/`P`（带 linewise/characterwise 感知的粘贴）、`>>` / `<<`（以 2 空格停止缩进/反缩进）、`o`/`O`（在下面/上面打开行并进入插入模式）。

**文本对象**找到光标周围的边界。它们回答问题："光标在里面的'东西'是什么？"

Word 对象（`iw`、`aw`、`iW`、`aW`）将文本分段为字形，分类每个为 word-character、whitespace 或 punctuation，并将选择扩展到词边界。`i`（inner）变体仅选择词。`a`（around）变体包含周围的空格——偏好尾随空格，行尾时回退到前导。大写变体（`W`、`aW`）将任何非空格序列视为词，忽略标点边界。

Quote 对象（`i"`、`a"`、`i'`、`a'`、`` i` ``、`` a` ``）在当前行找到成对引号。对按序匹配（第一个和第二个引号形成一对，第三个和第四个形成下一对，以此类推）。如果光标在第一个和第二个引号之间，那就是匹配。`a` 变体包含引号字符；`i` 变体排除它们。

Bracket 对象（`ib`/`i(`、`ab`/`a(`、`i[`/`a[`、`iB`/`i{`/`aB`/`a{`、`i<`/`a<`）进行深度追踪搜索匹配定界符。它们从光标向外搜索，维护嵌套计数，直到在深度零找到匹配对。这正确处理嵌套括号——在 `foo((bar))` 内的 `d i (` 删除 `bar`，不是 `(bar)`。
...
每个变更命令将自身记录为 `RecordedChange`——覆盖 insert、operator+motion、operator+textObj、operator+find、replace、delete-char、toggle-case、indent、open-line 和 join 的可辨识联合。`.` 命令从持久状态重放 `lastChange`，使用记录的计数、操作符和动作在当前光标位置重现完全相同的编辑。

Find-repeat（`;` 和 `,`）使用 `lastFind`。`;` 命令以相同方向重复上次查找。`,` 命令翻转方向：`f` 变为 `F`，`t` 变为 `T`，反之亦然。这意味着在 `fa`（查找下一个 'a'）之后，`;` 向前查找下一个 'a'，`,` 向后查找下一个 'a'——用户不需要记住他们正在搜索哪个方向。

寄存器追踪 yanked 和删除的文本。当寄存器内容以 `\n` 结尾时，它被标记为 linewise，这改变粘贴行为：`p` 插入在当前行之下（而非光标之后），`P` 插入在上方。此区分对用户不可见但对 vim 用户持续依赖的"删除一行，粘贴到别处"工作流至关重要。

---

## 虚拟滚动

长会话产生长对话。重度调试会话可能生成 200+ 条消息，每条包含 markdown、代码块、工具使用结果和权限记录。没有虚拟化，React 将在内存中维护 200+ 组件子树，每个带有自己的状态、effects 和 memoization 缓存。DOM 树将包含数千节点。Yoga 布局将在每帧上访问所有节点。终端将不可用。
...
- **每消息高度缓存**，终端列数改变时失效
- **用于转录搜索导航的跳转句柄**（跳转到索引、下一个/上一个匹配）
- **带热缓存支持的搜索文本提取**（当用户输入 `/` 时预小写所有消息）
- **粘性 prompt 追踪**——当用户从输入滚动离开时，他们最后的 prompt 文本出现在顶部作为上下文
- **消息动作导航**——用于回退功能的光标基消息选择

`useVirtualScroll` hook 基于 `scrollTop`、`viewportHeight` 和累积消息高度计算要挂载的消息。它在 `ScrollBox` 上维护滚动夹紧边界以防止爆裂 `scrollTo` 调用跑在 React 异步重渲染之前的空白屏幕——虚拟化列表的经典问题，其中滚动位置可能超过 DOM 更新。

虚拟滚动和 markdown token 缓存之间的交互值得注意。当消息滚动出视口时，其 React 子树卸载。当用户滚动回来时，子树重新挂载。没有缓存，这将意味着对用户滚动经过的每条消息重新解析 markdown。模块级 LRU 缓存（500 条目，按键为内容哈希）确保昂贵的 `marked.lexer()` 调用对每条独特消息内容最多发生一次，无论组件挂载和卸载多少次。

`ScrollBox` 组件本身通过 `useImperativeHandle` 提供命令式 API：

- `scrollTo(y)` — 绝对滚动，打破粘性滚动模式
- `scrollBy(dy)` — 累积到 `pendingScrollDelta`，由渲染器以受限速率排空
- `scrollToElement(el, offset)` — 通过 `scrollAnchor` 将位置读取推迟到渲染时间
- `scrollToBottom()` — 重新启用粘性滚动模式
- `setClampBounds(min, max)` — 约束虚拟滚动窗口

所有滚动变更直接到 DOM 节点属性并通过 microtask 调度渲染，绕过 React 的 reconciler。`markScrollActivity()` 调用通知后台间隔（旋转器、计时器）跳过其下一个 tick，减少活跃滚动期间的事件循环争用。这是一种合作调度模式：滚动路径告诉后台工作"我正处于延迟敏感的操作中，请 yield。"后台间隔在调度其下一个 tick 之前检查此标志，如果滚动活跃则延迟一帧。结果是即使多个旋转器和计时器在后台运行也能保持始终平滑的滚动。

---

## Apply This：构建上下文感知的 Keybinding 系统

Claude Code 的 keybinding 架构为任何带模态输入的应用——编辑器、IDE、绘图工具、终端多路复用器——提供了模板。关键洞察：

**将绑定与处理器分离。** 绑定是数据（哪个键映射到哪个动作名称）。处理器是代码（动作触发时发生什么）。将两者分开意味着绑定可以序列化为 JSON 供用户自定义，而处理器保留在拥有相关状态的组件中。用户可以重新绑定 `ctrl+k` 到 `chat:submit` 而不触及任何组件代码。

**Context 作为一等概念。** 不是一组平面的键映射，定义基于应用状态激活和停用的上下文。当对话框打开时，`Confirmation` 上下文激活且其绑定优先于 `Chat` 绑定。当对话框关闭时，`Chat` 绑定恢复。这消除了散布在事件处理器中的 `if (dialogOpen && key === 'y')` 条件杂烩。

**Chord 状态作为显式机器。** 多键序列（chords）不是单键绑定的特殊情况——它们是需要带有超时和取消语义的状态机的不同种类绑定。使其显式（带有专用的 `ChordInterceptor` 组件和 `pendingChordRef`）防止微妙 bug，其中 chord 的第二次击键被不同处理器消费，因为 React 的状态更新尚未传播。

**早期保留，清晰警告。** 在定义时识别不能被重新绑定的键（系统快捷键、终端控制字符），而非在解析时。当用户尝试绑定 `ctrl+c` 时，在配置加载期间显示错误，而非静默接受永不会触发的绑定。这是工作的 keybinding 系统和产生神秘 bug 报告的系统之间的区别。

**为终端多样性设计。** Claude Code 的 keybinding 系统在绑定级别定义平台特定的替代方案，而非处理器级别。图片粘贴是 `ctrl+v` 或 `alt+v` 取决于 OS。模式循环是 `shift+tab` 或 `meta+m` 取决于 VT 模式支持。每个动作的处理器相同，无论哪个键触发它。这意味着测试覆盖每个动作一个代码路径，而非每个平台-键组合一个。当新的终端怪癖出现时，修复是绑定定义中的单一条件，而非处理器代码中散布的 `if (platform === 'windows')` 检查。

**提供逃生口。** null-action 取消绑定机制小但重要。在终端多路复用器内运行 Claude Code 的用户可能发现 `ctrl+t`（切换 todos）与其多路复用器的 tab 切换快捷键冲突。通过向 keybindings.json 添加 `{ "ctrl+t": null }`，他们完全禁用绑定。键击透传到多路复用器。没有 null 取消绑定，用户的唯一选择是将 `ctrl+t` 重新绑定到某个他们不想要的其他动作，或重新配置他们的多路复用器——都不是好的体验。

Vim 模式实现添加了另一课：**让类型系统强制你的状态机**。12 变体 `CommandState` 联合使在 switch 语句中忘记状态不可能。`TransitionResult` 类型将状态变更与副作用分离，使机器可作为纯函数测试。如果你的应用有模态输入，将模式表达为可辨识联合，让编译器验证穷举性。定义类型所花的时间以消除的运行时 bug 来回报。

考虑替代方案：使用可变状态和命令式条件句的 vim 实现。`fromOperator` 处理器将是 `if (mode === 'operator' && pendingCount !== null && isDigit(key))` 检查的嵌套，每个分支修改共享变量。添加新状态（比如，宏录制模式）将需要审计每个分支确保新状态被处理。使用可辨识联合，编译器做审计——添加新变体的 PR 直到每个 switch 语句处理它才会构建。

这是 Claude Code 输入系统的更深层教训：在每一层——tokenizer、parser、keybinding resolver、vim 状态机——架构尽可能早地将非结构化输入转换为类型化的、穷举处理的结构。原始字节在解析器边界成为 `ParsedKey`。`ParsedKey` 在 keybinding 边界成为动作名称。动作名称在组件边界成为类型化处理器。每次转换缩小可能状态的空间，每次缩小由 TypeScript 的类型系统强制执行。到击键到达应用逻辑时，歧义已消失。没有"如果键是 undefined 怎么办？"没有"如果修饰键组合是不可能的怎么办？"类型已禁止这些状态存在。
...
这些原则不特定于终端应用。它们适用于任何必须跨多样化运行时环境处理高频输入并产生低延迟输出的系统。终端恰好是一个约束足够尖锐的环境，违反这些原则会产生立即可见的退化——掉帧、吞键击、闪烁。那种尖锐性使其成为优秀教师。

下一章从 UI 层移动到协议层：Claude Code 如何实现 MCP——让任何外部服务成为一等工具的统一工具协议。终端 UI 处理用户体验的最后一英里——将数据结构转换为屏幕上的像素和击键转换为应用动作。MCP 处理可扩展性的第一英里——发现、连接和执行活在 agent 自身代码库之外的工具。在它们之间，记忆系统（第 11 章）和 skills/hooks 系统（第 12 章）定义智能和控制层。整个系统的质量上限取决于所有四个：没有模型智能能补偿延迟的 UI，没有渲染性能能补偿无法到达所需工具的模型。
