...
UI 状态 store 大约 30 行：一个闭包包裹 `state` 变量、`Object.is` 相等性检查防止虚假更新、同步监听器通知，以及一个用于副作用的 `onChange` 回调：
...
三十四行。没有中间件、没有 devtools、没有时间旅行调试、没有 action 类型。只是一个闭包包裹的可变变量、一个 Set 监听器，以及一个 `Object.is` 相等性检查。这就是不用 Zustand 库但实现了 Zustand 风格的效果。
...
`onChange` 回调是两层同步的地方。每次 `setState` 调用都会触发 `onChangeAppState`，它接收先前和新状态并决定触发什么外部效果。

**权限模式同步**是主要用例。在集中化这个 handler 之前，权限模式只在 8+ 个变更路径中的 2 个同步到远程会话（CCR）。其他六个——Shift+Tab 循环、对话框选项、slash 命令、rewind、bridge 回调——都修改了 AppState 但没有告诉 CCR。外部元数据漂移失步。

修复方式：不要再把通知散落在各个变更点，而是集中在一个地方 hook diff。源代码中的注释列出了每个被破坏的变更路径，并指出"散落的调用点需要零改变。"这就是集中化副作用的架构优势——覆盖率是结构性的，而非手动的。

**模型变化**保持 bootstrap state 与 UI 渲染的内容同步。**设置变化**清除凭证缓存并重新应用环境变量。**Verbose 切换**和**展开视图**持久化到全局配置。

模式——在可 diff 的状态转换上集中化副作用——本质上是 Observer 模式，但应用在状态 diff 的粒度而非单个事件的粒度。它比散落的事件发射扩展性更好，因为副作用数量的增长远慢于变更点数量的增长。

> 💡 **译注**：这一段的核心思想是：不要在每个"修改状态的地方"分别写通知逻辑。而是反过来——在状态变化发生的地点集中监听，根据"新旧状态的差异（diff）"决定做什么。比如用户切换权限模式有 8 种方式（命令、快捷键、对话框、远程控制…），如果你在 8 个地方分别写"同步到远程会话"的逻辑，迟早会漏掉一个。如果在 store 的 `onChange` 里集中判断"权限模式变了→同步"，不管哪种方式改的，都会触发同步。这就是"结构性能覆盖所有变更路径"的意义。
