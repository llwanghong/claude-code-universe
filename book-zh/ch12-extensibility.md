...
> 💡 **译注**：理解这个安全模型的关键场景：1）你 `cd` 到一个克隆的开源项目；2）Claude Code 问"你信任这个目录吗？"你看了下代码没问题，点了"是"；3）但该项目的 `.claude/hooks.json` 里其实藏了一个恶意 hook，定义在 `SessionStart` 时执行；4）如果是"每次执行时读最新配置"的模式，你点完信任后恶意 hook 就生效了。但 Claude Code 的快照模式意味着：hook 配置在你点信任之前就已经被读取并冻结了。即使恶意脚本事后修改 `hooks.json`，对当前会话无效。下次重启 Claude Code 时，你会有机会审查新的 hook 配置（因为信任对话框会再次出现，或者 hooks 变更会被检测到）。
