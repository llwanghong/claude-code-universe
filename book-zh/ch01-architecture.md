...
以下是事后的解构——这六个抽象并不是事先在白板上设计出来的，而是在向大量用户交付生产级 agent 的压力中逐步涌现的。理解它们实际的样子（而非它们被计划的样子），为本书的其余部分设定了正确的基调：这不是一份理想化的架构文档，而是一份来自真实战场的战后报告。
...
第一，查询循环是一个 generator，不是回调链。REPL 通过 `for await` 从中逐条拉取消息，这意味着 backpressure（背压）是自然发生的——如果 UI 来不及渲染，generator 自动暂停，等 UI 处理完再继续产出。与之对比，事件发射器不管消费者能不能跟上都会持续推送，容易导致缓冲区溢出。Generator 模式是经过权衡后的有意选择。
