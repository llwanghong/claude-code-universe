...
│  ┌──────────────────────────────────────────────────────────────┐│
│  │  HEADER                                                      ││
│  │  [☰] [Project: my-app ▼]  [Session Tabs]    [⚙] [👤 User]  ││
│  └──────────────────────────────────────────────────────────────┘│
│  ┌────────────┬─────────────────────────────────────────────────┐│
...
│  │  │       │ │  │                                             │  ││
│  │  │ src/   │ │  │  ┌─────────────────────────────────────┐  │  ││
│  │  │ ├─auth│ │  │  │ 🤖 User: Fix the auth bug            │  │  ││
│  │  │ ├─api │ │  │  ├─────────────────────────────────────┤  │  ││
│  │  │ └─... │ │  │  │                                     │  │  ││
│  │  │       │ │  │  │  Claude: Let me look at this...     │  │  ││
│  │  └───────┘ │  │  │  ```diff                            │  │  ││
...
```
1. Agent 执行 Bash(git push origin main)
2. 工具流水线触发权限检查
3. 前端收到 permission_request 事件
4. PermissionDialog 弹出：
   ┌──────────────────────────────────────┐
   │  🔐 Permission Required              │
   │                                       │
   │  Command: git push origin main       │
   │  Risk: This will push to main branch │
   │                                       │
   │  [Allow Once] [Always Allow git*]    │
   │  [Deny]                              │
   └──────────────────────────────────────┘
5. 用户选择 → 发送 permission_response
6. Agent 继续执行
```
...
```
1. Agent 调用 DeployTool(production)
2. 触发审批流程
3. 前端收到 approval_pending 事件
4. ApprovalCard 显示：
   ┌──────────────────────────────────────┐
   ...
   │  Version: abc123def                  │
   │  Changes: fix null check in auth     │
   │  PR: #1234 (Build ✅ Tests ✅)        │
   │  Status: Waiting for TL approval...  │
   └──────────────────────────────────────┘
5. TL 通过 Slack/Jira/Web 审批
6. Agent 继续 → 执行部署
```
